...
MirrorMap* MirrorMap::Create(std::int32_t width, std::int32_t height)
{
    auto equirect = new MirrorMap();
    if (!equirect) {
        return nullptr;
...
bool MirrorMap::SaveAsPng(std::string_view filename) const
{
    AT_ASSERT(tex_);
