...
def exec_clang_tidy(file: str,  header_filter: str, will_use_subprocee = False):
    """Execute clang_tidy.
    ...
    if will_use_subprocee:
        proc = subprocess.Popen(clang_tidy_cmd,stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def exec_clang_tidy_by_compile_commands_json(compile_commands_json: str, ignore_words: str, header_filter: str = None, target_files: List[str] = []):
    ...
    with open(compile_commands_json, mode='r') as f:
        ...
        for item in compilation_db:
            ...
            if is_target:
                exec_clang_tidy(file, header_filter)

def main():
    # NOTE:
    # e.g.
    # python3 ./scripts/run_clang_tidy.py -i 3rdparty imgui unittest -f "${PWD}/aten/src/" -t accelerator.cpp
    parser = argparse.ArgumentParser(description="Run clang-tidy")
    parser.add_argument('-i', '--ignore_words', nargs='*', type=str, help="List to ignore. If file path include specified work, that file is ignored", default=[])
    parser.add_argument("-f", "--header_filter", type=str, help="Header filter for clang-tidy", default=None)
    parser.add_argument("-t", "--target_files", nargs="*", type=str, help="Specific target file to run clang-tidy", default=[])
