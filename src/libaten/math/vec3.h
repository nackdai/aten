...
namespace aten {
    ...
    inline AT_HOST_DEVICE_API vec3 vmin(const vec3& a, const float b)
    {
        return vec3(
            aten::min(a.x, b),
            aten::min(a.y, b),
    ...
    inline AT_HOST_DEVICE_API vec3 exp(const vec3& v)
    {
        return vec3(aten::exp(v.x), aten::exp(v.y), aten::exp(v.z));
    }
