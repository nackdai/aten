...
//
NvEdgeInfo * NvStripifier::FindEdgeInfo(
	...
	const unsigned int maxIndex)
{
	...
	for (int i = 0; i < maxIndex + 1; i++)
		edgeInfos[i] = NULL;
#else
	memset(&edgeInfos[0], 0, sizeof(NvEdgeInfo*) * edgeInfos.size());
#endif
	...
	{	
		...
		// grab the indices
		int v0 = m_Indices[index++];
		int v1 = m_Indices[index++];
		int v2 = m_Indices[index++];
...
//
NvFaceInfo* NvStripifier::FindGoodResetPoint(
	NvFaceInfoVec &faceInfos,
	NvEdgeInfoVec &edgeInfos)
{
	...
	if(result == NULL)
	{
		...
		} while (i != startPoint);	// １周するまで試してみる
...
{	
	...
	printf("GetNextIndex : shouldn't get here, but let's try and fail gracefully\n");
...
{
	// ストリップに所属する or お試し中でお試し中IDが一致する
...
inline void NvStripInfo::MarkTriangle(NvFaceInfo *faceInfo)
{
	...
	if (IsExperiment()){
		// お試し中
		faceInfo->m_experimentId = m_experimentId;	// お試し中ID
		faceInfo->m_testStripId  = m_stripId;		// このストリップのID
    }
	else{
		assert(faceInfo->m_stripId == -1);
		faceInfo->m_experimentId = -1;				// 正式であることをマーク
		faceInfo->m_stripId      = m_stripId;		// このストリップのID
	}
}
...
void NvStripInfo::Build(NvEdgeInfoVec &edgeInfos, NvFaceInfoVec &faceInfos)
{
...
#else
	tempAllFaces.resize(forwardFaces.size());
	memcpy(&tempAllFaces[0], &forwardFaces[0], sizeof(NvFaceInfo*) * forwardFaces.size());
#endif
	...
	while (nextFace != NULL && !IsMarked(nextFace))
	{
		//this tests to see if a face is "izique", meaning that its vertices aren't already in the list
...
#else
	m_faces.resize(backward.size() + forward.size());

	int numFaces = backward.size();
	for (int i = numFaces - 1; i >= 0; i--) {
		m_faces[numFaces - 1 - i] = backward[i];
	}

	memcpy(&m_faces[numFaces], &forward[0], sizeof(NvFaceInfo*) * forward.size());
#endif
}
	...
	NvFaceInfoVec& faceList)
{
	...
	{
		if(allStrips[i]->m_faces.size() < m_nMinStripLength)
		{
	...
	if(tempFaceList.size())
	{
		...
		VertexCache* vcache = new VertexCache(m_nCacheSize);
	...
	const unsigned int restartVal)
{
	...
	for (int i = 0; i < nStripCount; i++)
	{
		...
		// Handle the first face in the strip
		{
			...
			{
				// tFirstFaceの頂点の内strip->m_faces[1]に含まれないものを取得
...
#else
				bool b1 = NextIsCW(stripIndices.size() - accountForNegatives);
				bool b2 = IsCW(strip->m_faces[0], tFirstFace.m_v0, tFirstFace.m_v1);
				if (b1 != b2)
#endif
				{
					stripIndices.push_back(tFirstFace.m_v0);
...
#if 1
			int nShared0, nShared1;
			GetSharedVertices(strip->m_faces[j], &tLastFace, &nShared0, &nShared1);
#endif
...
#else
				int tmp_v0 = stripIndices[stripIndices.size() - 2];
				int tmp_v1 = stripIndices[stripIndices.size() - 1];

				if (strip->m_faces[j]->IsContain(tmp_v0, tmp_v1, nUnique)) {
					stripIndices.push_back(nUnique);
					...
					int tmp = (tmp_v1 == nShared0 ? nShared1 : nShared0);

					stripIndices.push_back(tmp_v1);
					stripIndices.push_back(tmp);
					stripIndices.push_back(tmp_v1);
					stripIndices.push_back(nUnique);

					// Update last face info
					tLastFace.m_v0 = tmp;
					tLastFace.m_v1 = tmp_v1;
					tLastFace.m_v2 = nUnique;
				}
#endif
			}
...
{
	m_fMeshJump = 0.0f;
	m_bFirstTimeResetPoint = true; //used in FindGoodResetPoint()

	//the number of times to run the experiments
	int numSamples = 10;
	
	//the cache size, clamped to one
	m_nCacheSize = max(1, in_cacheSize - CACHE_INEFFICIENCY);
	
	m_nMinStripLength = in_minStripLength;  //this is the strip size threshold below which we dump the strip into a list
	...
	int numSamples)
{
	...
	while (!done)
	{
		...
		// 最も効率よくストリップできたものを正式なストリップとして登録
