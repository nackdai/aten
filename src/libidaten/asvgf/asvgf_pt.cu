    ...
    float4* visibilityBuffer)
{
    ...
    do
    {
        ...
        if (isHit) {
            visibilityBuffer[idx].x = isect.a;
            visibilityBuffer[idx].y = isect.b;
            visibilityBuffer[idx].z = __int_as_float(isect.objid);
            visibilityBuffer[idx].w = __int_as_float(isect.primid);
        }
        else {
            // No hit.
            visibilityBuffer[idx].x = -1.0f;
            visibilityBuffer[idx].y = -1.0f;
            visibilityBuffer[idx].z = __int_as_float(idaten::AdvancedSVGFPathTracing::InvalidValue);
            visibilityBuffer[idx].w = __int_as_float(idaten::AdvancedSVGFPathTracing::InvalidValue);
        }
    } while (true);
}
