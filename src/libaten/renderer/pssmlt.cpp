...
#pragma omp parallel for
		...
		for (int mi = 0; mi < mltNum; mi++) {
			...
			for (int i = 0; i < seedPathMax; i++) {
				...
				// sum I.
				sumI += color::illuminance(sample.contrib);
			}
			...
			int selecetdPath = 0;
			{
				...
				for (int i = 0; i < seedPathMax; i++) {
					...
					accumlatedImportance += color::illuminance(path.contrib);
			...
			for (int i = 0; i < M; i++) {
				...
				real I = color::illuminance(newPath.contrib);
				real oldI = color::illuminance(oldPath.contrib);
